  ...
  public void startServer() {
    ...
    server.addDisconnectListener(client -> {
      ...
      if (userIdToRemove != null) {
        userSocketMap.remove(userIdToRemove);
        System.out.println("❌ User disconnected: " + userIdToRemove);
        System.out.println("📊 Remaining connected users: " + userSocketMap.keySet());
      } else {
        System.out.println("❌ Client disconnected: " + client.getSessionId() + " (not found in user map)");
      }
    });

    // Xử lý cuộc gọi
    server.addEventListener("callUser", CallData.class, (client, data, ackSender) -> {
      System.out.println("📥 Nhận yêu cầu gọi:");
      System.out.println(" - Từ user: " + data.getFrom());
      System.out.println(" - Gọi đến user: " + data.getUserToCall());
      System.out.println(" - Dữ liệu signal: " + data.getSignalData());
      ...
      if (receiver != null) {
        ...
        System.out.println("❌ Không tìm thấy user " + data.getUserToCall() + " trong danh sách kết nối!");
      }
    });

    // Xử lý trả lời cuộc gọi
    server.addEventListener("answerCall", AnswerData.class, (client, data, ackSender) -> {
      String toUserId = String.valueOf(data.getTo());
      System.out.println("📥 Nhận trả lời cuộc gọi:");
      System.out.println(" - Từ user: " + client.getSessionId());
      System.out.println(" - Gửi đến user: " + toUserId);
    ...
    server.addEventListener("endCall", CallData.class, (client, data, ackSender) -> {
      System.out.println("📞 Nhận yêu cầu kết thúc cuộc gọi từ user: " + data.getUserId());
      System.out.println("📞 Call ID: " + data.getCallId());
      ...
      if (receiver != null) {
        receiver.sendEvent("callEnded", data.getCallId());
        System.out.println("📞 Đã thông báo kết thúc cuộc gọi đến user: " + data.getUserId());
      } else {
        System.out.println("❌ Không tìm thấy user " + data.getUserId() + " trong danh sách kết nối!");
      }
    });
    ...
    server.addEventListener("iceCandidate", Map.class, (client, data, ackSender) -> {
      ...
      if (receiver != null) {
        ...
        System.out.println("❌ Receiver " + to + " not found for ICE candidate!");
      }
    });
